...
<li><p>Presented at <a href="https://juliacon.org">JuliaCon</a> 2021, available at <a href="https://jcarpinelli.dev/juliacon">jcarpinelli.dev/juliacon</a></p>
</li>
</ul>
</div>
"""
...
# ╔═╡ acfd7a7b-2be3-444b-b2cc-b5a25db11159
begin
	orbit, T = halo(SunEarth; Az=100_000u"km", L=2)
	manifold = stable_manifold(orbit, T; num_trajectories = 50, duration=T, eps=-1e-7)

	mat = reduce(hcat, map(step -> vcat(position_vector(step[end]), velocity_vector(step[end])), manifold)) |> transpose |> copy
	
	@terminal @show mat[35,:] |> vec
